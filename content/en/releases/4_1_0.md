...
The bulk delete of Reports tab has been redesigned to allow the user to select multiple reports and delete them. You can read more about it in the [Reports tab docs](/building/reports/reports-overview/#bulk-delete-reports).
...
Offline users can now be replaced on a device so that a new user can use that device without needing to immediately sync with the server. You can read more about it in [the transitions configuration docs](/building/reference/app-settings/transitions#create_user_for_contacts).
