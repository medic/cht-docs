...
See the [workflow documentation](/building/login#password-reset-on-first-login) for more details.

## Highlights

### Change password on first login

System admin users create accounts for CHWs and then share the password with them. To enhance the security of these accounts, the CHT now [supports a feature](/building/login#password-reset-on-first-login) that prompts users to change their password upon their first login.
...
- This requirement can be bypassed for specific user roles using the `can_skip_password_change` [permission](/building/reference/app-settings/user-permissions#system-defined-permissions).
