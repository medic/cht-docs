...
4. Notify your Kubernetes Admin to create your namespace, and add your role and rolebindings that tie into the aws-auth configmap in the kube-system namespace.

5. Now is a good time to test all your access works correctly with both `kubectl` and `helm`:
   ```shell
   kubectl -n <username>-dev get pods
   helm list -n <username>-dev
   ```

   Which should return empty responses with no errors:
   
   ```shell
   No resources found in <username>-dev namespace. # kubectl response
   
   NAME	NAMESPACE	REVISION	UPDATED	STATUS	CHART	APP VERSION # helm response
   ```

6. Create a new `values.yaml` file by [saving a local copy of this one](https://raw.githubusercontent.com/medic/helm-charts/refs/heads/main/charts/cht-chart-4x/values.yaml). 
7. Edit `values.yaml` after you create it:
