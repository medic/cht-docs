...
{{< tabs items="Production,Development" >}}
{{< tab >}}
Be sure to replace `SECRET` with the value from [the step above](/hosting/sso/keycloak/#copy-secret). As well, replace `CHT_URL` with the production URL of your CHT instance, `USER` with your admin user and `PASSWORD` with your password:
```shell
...
{{< /tab >}}
{{< tab >}}
Be sure to replace `SECRET` with the value from [the step above](/hosting/sso/keycloak/#copy-secret). As well, replace `CHT_URL` with the docker helper URL, including port:
```shell
