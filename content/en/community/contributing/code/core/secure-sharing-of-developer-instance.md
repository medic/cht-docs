...
> 
> Also not - if you only want to test with mobile devices that require a valid TLS certificate, do not use method.  Instead, use [nginx-local-ip](/community/contributing/code/core/dev-environment#nginx-local-ip)

## Overview
When using a local [development environment](/community/contributing/code/core/dev-environment), you may want to share your work with other collaborators. By using a publicly accessible web server, you can receive the secure https requests and forward them back to your CHT instance which doesn't have https set up:

{{< figure src="/community/contributing/code/troubleshooting/obtaining-logs/SSH-tunnel-diagram.svg" caption="Path data flows from mobile device -> reverse proxy -> router -> developer" >}}
...
This guide assumes:
* You have a local [dev instance](/community/contributing/code/core/dev-environment) set up of cht-core 
...
{{< figure src="/community/contributing/code/troubleshooting/obtaining-logs/ssh-ports.svg" caption="Explanation of ports and IPs from SSH" >}}
