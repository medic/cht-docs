...
|`validations`|A set of validations to perform on incoming reports. Read more information about [configuring validation rules](app-settings-validations.md).|no|
...
|`messages[].event_type`|An event that will trigger sending of this message. Typical values are: `report_accepted` when the report has been successfully validated, `registration_not_found` when the case ID supplied in the report doesn't match any case ID issued by Medic. Read the [documentation on muting](transitions.md#muting).|no|
