...
If setting up CHT 3.x, you need to harden CouchDB with a `grunt` call, required even in development:
...
If setting up CHT 4.x, you need to startup CouchDB:
...
- (optionally) `API_PORT`: the port API will run on. If not defined, use `5988`
