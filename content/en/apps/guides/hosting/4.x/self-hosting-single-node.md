...
3. `couchdb` - the path for the docker-compose file of the upgrade-service
...
* `COUCHDB_SECRET` and `COUCHDB_PASSWORD` - can easily be generated with the `shuf` command using the `words` file:
   ```shell
   shuf -n7 /usr/share/dict/words --random-source=/dev/random | tr '\n' '-' | tr -d "'" | cut -d'-' -f1,2,3,4,5,6,7
  ```
