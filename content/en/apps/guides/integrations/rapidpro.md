...
### Lessons from integrations with the CHT

The following are best practices when working on a RapidPro deployment to integrate with the CHT. Review RapidPro’s documentation to familiarize yourself with various components that include the [API](https://rapidpro.io/api/v2/). The next sections will focus on various components or stages in the deployment process.
