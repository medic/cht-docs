...
Then, it will iterate over all existent contacts, collecting all reports about that contact along with all sms messages that the contact has sent or received. This is similar to the scoping you may have encountered when configuring [tasks]({{<ref "apps/reference/tasks" >}}) and [targets]({{<ref "apps/reference/targets" >}}).
...
- `chtScriptApi` the CHT API that provides CHT-Core Framwework's functions to other parts of the app. More info on the API can be found [here](https://docs.communityhealthtoolkit.org/apps/reference/_partial_cht_api/)
