...
1. Allow your CouchDB to be accessible via `https`. One way is to run `ngrok http XXXX`. Here `XXXX` represents CouchDB HTTPS port number _(it's `5984` by default)_ which can be found in the [database configuration](https://docs.couchdb.org/en/stable/config/http.html#chttpd/port) under `chttpd`. This will make your CouchDB accessible via a url like `https://abcd1234.ngrok.io`.
