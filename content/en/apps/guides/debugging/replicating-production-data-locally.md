...
1. Allow your CouchDB to be accessible via `https`. One way is to run `ngrok http PORT`. The `PORT` number can be found in database configuration using fauxton under the `chttpd` section. _(it's `5984` by default)_. This will make your CouchDB accessible via a url like `https://abcd1234.ngrok.io`.
