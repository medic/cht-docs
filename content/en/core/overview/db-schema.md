...
The `type` property of contact records depends on the version of Medic you are running:
 - If you are running 3.7 or later you get to [configure your contact hierarchy]({{< ref "building/application-settings/app-settings/hierarchy#app_settingsjson-contact_types" >}}), and the `type` of contacts is `contact`, and the configured type is in the `contact_type` property.
...
SMS forms are defined in [application config]({{< ref "building/application-settings/app-settings/patient_reports#app_settingsjson-patient_reports" >}}).
...
[Partner configuration code]({{< ref "building/tasks/tasks-js" >}}) running inside the Core Framework can cause tasks to appear within the Tasks tab. Each task in the tab is powered by a task document. Task documents are:
...
## Targets
[Partner configuration code]({{< ref "building/targets/targets-js" >}}) can configure targets to appear within the Targets/Analytics tab. Target documents are:
