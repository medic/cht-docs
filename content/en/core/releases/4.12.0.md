...
- `can_view_old_filter_and_search` (introduced in [`3.17.0`]({{% ref "core/releases/3.17.0.md" %}}))
- `can_view_old_action_bar` (introduced in [`4.2.0`]({{% ref "core/releases/4.2.0.md" %}}))
While it’s not necessary to remove these permissions from your [Roles]({{< relref "building/app-settings/app-settings-json/user-roles" >}}), they will no longer be recognized by the CHT and users will automatically see the updates they relate to.
