...
The registration transition now supports storing a phone number field for new patients. This is achieved through an additional field in the SMS form, which can be configured to be parsed and validated as a correct and, optionally unique, phone number. [links to documentation]
...
A new API endpoint now allows exporting API request performance. The output formatted for the Prometheus Data Model and is available without requiring authentication. This endpoint is used by CHT-Watchdog. [More details are available in API reference docs]({{< relref "apps/reference/api#get-apiv1express-metrics" >}}).
