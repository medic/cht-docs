...
We can now configure start_from to accept array of fields while configuring schedules. This allows users to pass multiple fields and first existing field will be used to create schedules. Read more about schedules in [documentation](https://docs.communityhealthtoolkit.org/apps/reference/app-settings/schedules/#app_settingsjson-schedules). cht-conf 3.20.0 or higher is required to validate config containing arrays of field in start_from. 

### Target components are updated to match material design

We updated target components to match to [material design](https://m3.material.io/components/dialogs/guidelines). Many other components were updated in [4.4.0](https://docs.communityhealthtoolkit.org/core/releases/4.4.0/#uiux-changes).


{{< figure src="../images/4.5.0-8513-target-changes" class="left col-12 col-lg-12" >}}
<br clear="all">

### Outbound tasks can be configured on a schedule
CHT now supports adding cron job on outbound push so that tasks can be run on a schedule based on the cron expression. See more about this [here]({{< relref "apps/reference/app-settings/outbound" >}})
...
- [#8564](https://github.com/medic/cht-core/issues/8564): Security issue #453
...
- [#7148](https://github.com/medic/cht-core/issues/7148): Feedback docs: "409: Document update conflict"
