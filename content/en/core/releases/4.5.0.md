...
The targets UI has been updated to match [material design](https://m3.material.io/components/dialogs/guidelines). This is the next step in transitioning to using a modern and intuitive design throughout the UI. Many other components were updated in [4.4.0](https://docs.communityhealthtoolkit.org/core/releases/4.4.0/#uiux-changes).
...
The CHT now supports scheduling when outbound push tasks are run based on a cron expression. Read about how to set this up in the [outbound push documentation]({{< relref "apps/reference/app-settings/outbound" >}}).
