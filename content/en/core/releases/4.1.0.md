...
Offline users can now be replaced on a device so that a new user can use that device without needing to immediately sync with the server. You can read more about it in [the docs]({{< ref "apps/reference/app-settings/transitions/#create_user_for_contacts" >}}).

### Redesigned bulk delete of reports redesign

The bulk delete of Reports tab has been redesigned to improve the user experience. Check out the new UI in the [Reports tab docs]({{< ref "apps/features/reports/#bulk-delete-reports" >}}).
