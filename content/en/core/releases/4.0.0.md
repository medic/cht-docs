...
This is a significant change how the CHT is deployed so the upgrade process requires some manual steps. [Documentation is coming soon]({{< ref "apps/guides/hosting/4.x" >}}), but in the meantime please reach out on [the forum](https://forum.communityhealthtoolkit.org/) for direct support in upgrading. New projects should start on 4.0.0 to make the following upgrades easier.
