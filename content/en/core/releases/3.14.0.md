...
Now the application has been updated to also use the Bikram Sambat format when displaying dates in places such as the contact summary.
...
We have updated the tab labels to always display (even on small screens). Additional the size of the label text can scale so that the text will display properly even when the tabs are small.
...
- If a contact has more than 20.000 records, it will be skipped (none of its records will be purged). 
