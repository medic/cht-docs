...
|sms.clear_failing_schedules            | When `true`, messages that can not be set to `pending` state for various reasons are set to `clear`. This allows messages that were not sent in the past to clear from the instance. | false | 5.1.0 | 
