...
When using SSO Login, the [`app_url` setting](/building/reference/app-settings/#app_settingsjson) must also be defined. Also, your CHT application's _client secret_ from the OIDC Provider must be securely configured in your CHT instance with the [Credentials API](/building/reference/api/#/Config/v1CredentialsKeyPut) using the key `oidc:client-secret`.

Only users with the [`oidc_username` property](/building/reference/api/#/User/v3UsersPost) can log in with SSO.
