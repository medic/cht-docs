...
| `profile_image_field` | The name of the contact field containing the profile image. When an image is uploaded for this field on a contact (e.g. in the contact's create form), the header on the contact's profile page will display the image. If the contact does not have a photo set for this field, the contact type `icon` will be displayed. Added in 5.3.0.                         | No, defaults to `profile_image`.   |
