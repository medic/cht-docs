...
Sending patients, patient contacts, and reports data can be achieved using the [Outbound push](/building/reference/app-settings/outbound). Receiving data from OpenMRS can be achieved using the CHT Core Web [API](/building/reference/openapi/).
...
You may want to further configure a service that relays feedback to the CHT. Feel free to utilize in-app text messages, which can be triggered via the [sms endpoint](/building/reference/openapi/#/SMS/smsPost) of CHT. The health workers would receive these feedback messages on their phones as well as access via the Messages tab in-app.
...
1. Do patient matching to avoid duplicate details. Querying can be achieved via available endpoints such as `contacts_by_phone` [endpoint](/building/reference/openapi/#/Contact/v1ContactsByPhoneGet) and `hydrate` [endpoint](/building/reference/openapi/#/Bulk/v1HydrateGet) among others/
