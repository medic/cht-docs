...
|3|RapidPro|`POST` to the [records endpoint](/building/reference/openapi/#/SMS/v2RecordsPost) in the CHT.  If you used the Global value mentioned above, the POST will look something like `@globals.api/v2/records`.|
...
Another common action you will likely need to perform in RapidPro is getting information from the CHT about a user or patient based on their phone number. You can use the [contacts-by-phone](/building/reference/openapi/#/Contact/v1ContactsByPhoneGet) API to get fully hydrated contacts associated to that phone number.
