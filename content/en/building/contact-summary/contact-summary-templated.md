...
| `uhcStats.uhcInterval` | Object containing the start and end date of UHC reporting period, it is calculated from the `uhc.visit_count.month_start_date` defined in the [app settings]({{< ref "/building/app-settings/app-settings-json/#app_settingsjson" >}}). |
...
| `uhcStats.homeVisits` | Object containing the contact's home visits stats. The [contact's type]({{< ref "/building/app-settings/app-settings-json/hierarchy.md#app_settingsjson-contact_types" >}}) should have `count_visits` enabled and the [UHC visit count settings]({{< ref "/building/app-settings/app-settings-json/#app_settingsjson" >}}) should be defined, additionally this information is only available for users that have `can_view_uhc_stats` permission and that are not System Administrators. |
...
| `uhcStats.homeVisits.countGoal` | Number, home visits goal, defined in the [UHC visit count settings]({{< ref "/building/app-settings/app-settings-json/#app_settingsjson" >}}). |
