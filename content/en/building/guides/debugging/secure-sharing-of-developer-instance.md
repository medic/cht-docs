...
Also not - if you only want to test with mobile devices that require a valid TLS certificate, do not use method.  Instead, use [nginx-local-ip]({{< relref "community/code/core/dev-environment#nginx-local-ip" >}})
{{% /alert %}}

## Overview
When using a local [development environment]({{< relref "community/code/core/dev-environment" >}}), you may want to share your work with other collaborators. By using a publicly accessible web server, you can receive the secure https requests and forward them back to your CHT instance which doesn't have https set up:
...
This guide assumes:
* You have a local [dev instance]({{< relref "community/code/core/dev-environment" >}}) set up of cht-core 
