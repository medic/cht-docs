...
NOTE: The bottom action bar can be temporarily re-enabled for specific users by granting them the `can_view_old_action_bar` permission; however, it will be completely removed in a future release. See the [Feature Flags]({{< relref "building/updates/feature-flags" >}}) documentation for more information.
