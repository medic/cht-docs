...
Contacts are organized into a hierarchy. It is not straight-forward to move contacts from one position in the hierarchy to another because many copies of this hierarchy exist. Use the `move-contacts` action in [`cht-conf`](https://github.com/medic/cht-conf) to assign a new parent to contacts.

{{< callout type="info" >}}
From 5.3.0 there is also a [server API for moving and deleting contacts](/building/contact-management/hierarchy-operations-api), which does the same work without requiring command line access. This page describes the `cht-conf` action, which remains available and is still the better fit for moving several unrelated contacts at once or reviewing the changed documents before uploading them.
{{< /callout >}} This command will move the specified contact, all the contacts under that contact, and all reports created by any of those contacts. This action will download all documents that need to be updated, update the lineages within those documents, and then save the updated documents on your local disk. To commit those changes to the database, run the `upload-docs` action.
