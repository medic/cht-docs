...
1. Assuming your IP is `192.168.0.3`, start `nginx-local-ip` to connect to:
   * The CHT API running via `grunt` or `horti`, execute `APP_URL=http://192.168.0.3:5988 docker compose up` and then access it at `https://192-168-0-3.my.local-ip.co/`.
   * The CHT API running via `docker`, the ports are remapped, so execute `HTTP=8080 HTTPS=8443 APP_URL=https://192.168.0.3 docker compose up` and then access it at `https://192-168-0-3.my.local-ip.co:8443/`.
