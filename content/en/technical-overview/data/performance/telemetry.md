...
| `tasks:all-tasks`                                                  | The number of tasks visible to the user, recorded on every recalculation. Added in 5.1.0 | |
...
| `enketo:external-app:<action>`                                     | The time taken from launching an external Android app to receiving the response. The `action` is the intent action of the launched app (e.g. `android.media.action.IMAGE_CAPTURE`). Added in 5.2.0. | |        
...
To get summary data with one item per user, see the `/api/v2/export/user-devices` [API](/building/reference/api/#/Export/v2ExportUserDevicesGet), which was added in CHT 4.7.0. 
